...
class FragmentRegister : Fragment() {
        ...
        binding.btnSignup.setOnClickListener {
            ...
            if (validateInput(name, input, password, passwordRestype)) {
                ...
                if (isEmail(input)) {
                    ...
                    val normalizedPhone = normalizePhoneNumber(input)
                    inputPhoneNumber = normalizedPhone
    ...
    private fun normalizePhoneNumber(phone: String): String {
        ...
        return when {
            normalized.startsWith("+") -> normalized     // Đã đúng dạng (+84...)
            normalized.startsWith("84") -> "+$normalized" // Dạng 84...
            normalized.startsWith("0") -> "+84${normalized.substring(1)}" // Dạng 0...
            else -> "+84$normalized" // Dạng không có 0 (356...)
        }
    }
                ...
                if (task.isSuccessful) {
                    ...
                    firebaseUser?.sendEmailVerification()
                        ?.addOnSuccessListener {
                            Toast.makeText(requireContext(), "Đăng ký thành công! Vui lòng kiểm tra email để xác thực tài khoản.", Toast.LENGTH_LONG).show()
                        }
                        ?.addOnFailureListener { e ->
                            Log.e("Register", "Lỗi gửi email xác thực: ${e.message}")
                            Toast.makeText(requireContext(), "Lỗi gửi email xác thực.", Toast.LENGTH_SHORT).show()
                        }
                    saveAllUserData()
                    ...
                    try {
                        ...
                        //Email đã tồn tại
                        Toast.makeText(requireContext(), "Địa chỉ email này đã được sử dụng.", Toast.LENGTH_SHORT).show()
                        binding.edtEmailSignup.error = "Email đã tồn tại"
                        binding.edtEmailSignup.requestFocus()
                    } catch (e: com.google.firebase.auth.FirebaseAuthWeakPasswordException) {
                        // Mật khẩu yếu
                        Toast.makeText(requireContext(), "Mật khẩu quá yếu. Vui lòng sử dụng ít nhất 6 ký tự.", Toast.LENGTH_SHORT).show()
                        binding.edtPasswordSignup.error = "Mật khẩu phải có ít nhất 6 ký tự"
                        ...
                        binding.edtEmailSignup.error = "Email không hợp lệ"
                        binding.edtEmailSignup.requestFocus()
                    }
                    catch (e: Exception) {
                        Log.e("Register", "Đăng ký thất bại: ${e.message}")
